...
Canonical validation helpers for assembled echo stats.

These helpers validate the parsed ``stats`` dict produced at the
service/assembler boundary. Legacy processing code may still import them via
the compatibility wrapper in ``scraping.processing.echoesValidator``.
...
def _load_spec() -> dict:
    yaml_path = Path(__file__).resolve().parent.parent / 'processing' / 'echo_stats_valid_values.yaml'
    try:
...
def validate_echo_stats(
    ...
    rolled_candidates = [key for key in main if key != fixed_name]
    ...
    seen_base_flat: set[str] = set()
    seen_base_pct: set[str] = set()
    ...
    for name, value in sub.items():
        ...
        _check_substat_value(result, name, actual_value, [float(tier) for tier in valid_tiers])

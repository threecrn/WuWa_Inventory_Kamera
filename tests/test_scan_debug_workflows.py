...
def test_home_manual_update_uses_local_state(monkeypatch) -> None:
    saved_calls: list[tuple[dict[str, tuple[object, type]], str]] = []
    notifications: list[tuple[str, str, str]] = []

    item_key, item_info = next(
        (key, info)
        for key, info in home_module.itemsID.items()
        if info['name'].lower().replace(' ', '') == key
    )
    ...
    class _FakeListWidget:
        def currentItem(self) -> _FakeListItem:
    ...
    home = SimpleNamespace(
        ...
        owned_spinbox=_FakeSpinBox(),
        updateUISignal=_FakeSignal(),
        _manual_inventory_items={},
        _manual_failed_items=[{'image': 'unused.png', 'owned': 1}],
        _manual_session_id='session-id',
        showNotification=lambda level, title, message: notifications.append((level, title, message)),
    )

    home_module.HomeInterface.onChangeButtonClicked(cast(Any, home))

    assert item_key == item_info['name'].lower().replace(' ', '')
    assert home._manual_inventory_items == {item_info['id']: 7}
    assert home._manual_failed_items == []
    assert home.updateUISignal.calls == 1
    assert saved_calls == [
        (
            {
